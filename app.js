...
(function() {
    ...
    // This URL tells the frontend to make API calls to the same server
    ...
    function route() {
        ...
        if (titleEl) titleEl.textContent = titleFromPath(path);
        const mount = $('#app');
        if (!mount) return;
        mount.innerHTML = '';
        view();
    ...
    function highlightNav(path) {
        ...
        const current = $(`.nav-item[data-route="${path}"]`);
    ...
    function syncUserUI() {
        ...
        const signOut = byId('signOutBtn');
        if (signOut) signOut.style.display = isAuthed ? 'flex' : 'none';
        const nameEl = byId('userName');
        if (nameEl) nameEl.textContent = isAuthed ? (state.user.name || 'User') : 'Guest';
        const avEl = byId('userAvatar');
        if (avEl) avEl.textContent = (isAuthed ? (state.user.name || 'U') : 'Q').slice(0, 1).toUpperCase();
        const loginNav = document.querySelector('.nav-item[data-route="login"]');
        if (loginNav) loginNav.style.display = isAuthed ? 'none' : 'flex';
    }
    ...
    function renderDashboard() {
        ...
        clearVidBtn.addEventListener('click', () => { video.removeAttribute('src'); video.load(); byId('vidResult').innerHTML = ''; analyzeVidBtn.disabled = true; clearVidBtn.disabled = true; vidInput.value = ''; currentVideoFile = null; });

        drawGlobalHeatmap();
    }

    function renderLogin() {
        const tpl = byId('view-login');
        $('#app').appendChild(tpl.content.cloneNode(true));
        const tabBtns = $$('.tab');
        ...
        byId('signinBtn').addEventListener('click', async () => {
            ...
            try {
                ...
                setUser({ name: byId('signupName').value.trim() || email.split('@')[0], email: email, bio: '' });
        ...
        byId('signupBtn').addEventListener('click', async () => {
            ...
            if (!email || !password) return toast('Please fill in email and password');
            try {
                const response = await fetch(`${API_BASE_URL}/signup`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ email, password }) });
                const data = await response.json();
        ...
        function selectTab(name) {
            ...
            $$('.tab-content').forEach(el => el.classList.toggle('show', el.id === 'tab-' + name));
        }
    }

    function renderProfile() {
        if (!requireAuth()) return;
        const tpl = byId('view-profile');
        $('#app').appendChild(tpl.content.cloneNode(true));
        const u = state.user;
        byId('profileAvatar').textContent = u.name.slice(0, 1).toUpperCase();
        byId('profileName').value = u.name;
        byId('profileEmail').value = u.email;
        byId('profileBio').value = u.bio || '';
        byId('saveProfileBtn').addEventListener('click', () => {
            const name = byId('profileName').value.trim() || 'User';
            const bio = byId('profileBio').value;
            setUser({ ...state.user, name, bio });
            toast('Profile saved');
        });
        byId('resetProfileBtn').addEventListener('click', () => route());
    }

    function renderHistory() {
        const tpl = byId('view-history');
        $('#app').appendChild(tpl.content.cloneNode(true));
        buildHistoryList(state.history);
        byId('exportHistoryBtn').addEventListener('click', () => {
            const blob = new Blob([JSON.stringify(state.history, null, 2)], { type: 'application/json' });
            const a = document.createElement('a');
            a.href = URL.createObjectURL(blob);
            a.download = 'qodo-history.json';
            a.click();
        });
        byId('clearHistoryBtn').addEventListener('click', () => {
            setHistory([]); buildHistoryList([]); drawGlobalHeatmap();
        });
    }

    function renderHelp() {
        const tpl = byId('view-help');
        $('#app').appendChild(tpl.content.cloneNode(true));
    }
    ...
    // Helper functions
    function renderBackendResult(analysis) {
        const confidence = (analysis.confidence * 100).toFixed(1);
        const severity = analysis.verdict || 'unknown';
        const explanation = analysis.explanation || 'No explanation provided.';
        return `<div>Verdict: <b>${severity.toUpperCase()}</b> • Confidence: <b>${confidence}%</b></div><div style="margin-top:8px; color: var(--muted);">${escapeHtml(explanation)}</div>`;
    }
    function drawGlobalHeatmap() {
        const canvas = byId('globalHeatmap');
        if (!canvas) return;
        ...
        const cols = 30; const rows = 8;
        const cellW = w / cols; const cellH = h / rows;
        for (let y = 0; y < rows; y++) {
            for (let x = 0; x < cols; x++) {
                const v = data[(y * cols + x) % data.length] || 0;
                const c = mixColor([34, 211, 238], [167, 139, 250], v);
                ctx.fillStyle = `rgba(${c[0]},${c[1]},${c[2]}, ${0.1 + v * 0.7})`;
                roundRect(ctx, x * cellW + 2, y * cellH + 2, cellW - 4, cellH - 4, 4);
                ctx.fill();
            }
        }
        const summary = byId('globalSummary');
        if (summary) summary.textContent = summarizeHistory();
    }
    ...
    function aggregateHistory() {
        if (!state.history.length) return new Array(60).fill(0.05);
        const severityMap = { low: 0.2, caution: 0.2, medium: 0.5, misinformation: 0.9, high: 0.9, safe: 0.1 };
        return state.history.map(h => severityMap[h.severity] || 0.1).slice(-240);
    }
    function summarizeHistory() {
        const c = { msg: 0, img: 0, vid: 0 };
        state.history.forEach(h => c[h.type]++);
        const total = state.history.length;
        return `Total: ${total} • Msg: ${c.msg} • Img: ${c.img} • Vid: ${c.vid}`;
    }
    function pushHistory(entry) {
        const record = { id: Date.now(), ts: new Date().toISOString(), ...entry };
        const next = [record, ...state.history].slice(0, 300);
        setHistory(next);
        if (location.hash === '#/history') buildHistoryList(next);
    }
    function buildHistoryList(list) {
        ...
        list.forEach(item => {
            ...
            const badge = item.type === 'msg' ? 'msg' : item.type === 'img' ? 'img' : 'vid';
            el.innerHTML = `<div><span class="badge ${badge}">${item.type.toUpperCase()}</span><span class="muted"> • ${new Date(item.ts).toLocaleString()}</span><div class="muted small">${escapeHtml((item.input || '').toString()).slice(0, 120)}</div></div><div class="severity">${(item.severity || 'N/A').toUpperCase()}</div>`;
            container.appendChild(el);
        });
    }
    function filterHistory(q) {
        const filtered = state.history.filter(h => (h.input || '').toLowerCase().includes(q));
    ...
    function toast(msg) {
        ...
        if (!el) {
            ...
            el.style.cssText = 'position:fixed; right:16px; bottom:16px; padding:12px 14px; border:1px solid var(--line); border-radius:10px; background:var(--elev); box-shadow:var(--shadow); color:var(--text); transition: opacity 0.3s;';
